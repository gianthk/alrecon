...
class slurmjob:
    ...
    def set_recon_command_beats(self, alrecon_state):
        '''Assemble reconstruction python command for reconstruction call on rum.sesame.org.jo using the python script BEATS_recon.py. For integration with a different HPC reconstruction environment you will have to modify the python call generated by this method. Tomopy reconstruction calls at SESAME BEATS are  handled through the script BEATS_recon.py contained in:
            https://github.com/SESAME-Synchrotron/BEATS_recon/blob/master/scripts/rum/BEATS_recon.py
